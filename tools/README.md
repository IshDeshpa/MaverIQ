...
1. **Measurement Study, Characterization and Profiling:**
    ...
    - `full_model_data_collection_BS_all.py`: Script to collect ground truth information for GPT-J-6B and FALCON-40B for multiple output lengths and batch sizes --> Output: `MaverIQ//outputs/profiling_data/full_model_data_collection_BS.csv`
    ...
    - `full_model_data_collection_prunning.py`: Script to collect ground truth information for pruned models --> Output: `MaverIQ/outputs/profiling_data/full_model_data_collection_prunning.csv`
    ...
    - `full_model_data_collection.py`: Script to collect ground truth information for all the models with multiple output lengths (batch size = 1) --> Output: `MaverIQ//outputs/profiling_data/full_model_data_collection.csv`
...
2. **Profiling:**
    ...
    - `profiler.py` : Script to collect only the required metadata for fingerprint profiling following `fingerprint_data_collection_BS.py` implementation. Usage:
        ...
        ```
        This script when executed, stores the results in `profiling_data_<model_name>.csv` within the working working directory, however for our calculations we use the results stored in `MaverIQ/outputs/profiling_data` which contain the same information.
...
3. **Estimation:**
    ...
    - `GD_algorithm.py`: Script to perform the Gradient Descent algorithm to calculate the constant parameters --> Output: `MaverIQ/outputs/estimator_data/latency_exponents.csv`
...
4. **Runtime Controller:**
    ...
    - `AlpaServe_evaluation.sh`: Bash scripts that contains all the commands used for the end-to-end experiments using AlpaSeve and AlpaServe*.
    - `MaverIQ_evaluation.sh`: Bash scripts that contains all the commands used for the end-to-end experiments using MaverIQ.
    - `model_load_sequence.csv`: Example file with model-list used by `MS_external_access_baselines.py` and `MS_external_access.py`.
    - `MS_client.py`: Script used to serve the incoming requests and send them to correct clients.
    - `MS_controller_baselines.py`: MaverIQ's controller used to orchestrate the whole runtime execution.
    - `MS_controller.py`: Baseline's controller used to orchestrate the whole runtime execution.
    ...
    - `MS_model_acc_MMLU.json`: Json file that stores the accuracy-performance of each model based the MMLU benchmark. Those number are based on the model's FP16-accuracy (i.e. FP16-configurations get 100%). Users must manually specify those number when registering a model. A script to run and collect those results can be found in `MaverIQ/TensorRT-LLM/examples/mmlu.py`. First the FP16 accuracy is calculated, then for all other configurations we execute the script and report the comparative accuracy to FP16 (e.g. for INT-8 configurations we store `100*<INT8-Accuracy>/<FP16-Accuracy>`).
    ...
    - `MS_trace_generator_baselines.py`: Script the generates the scaled Azure trace and initializes inference serving for AlpaServe and AlpaServe*. Usage:
        ```bash 
        ...
        ```
    - `MS_trace_generator.py`: Script the generates the scaled Azure trace and initializes inference serving for MaverIQ.
        ```bash 
...
5. **Artifacts Plotting:**
    We provide the Jupyter notebook used for plotting and analyzing the results presented in the paper. This file is located in `MaverIQ/tools/artifacts-plotting`:
...
```bash
|-- tools                                                   : Scripts for MaverIQ implementation
...
|   |   |-- memory_estimator.py                             
|   |-- measurment                                          : Scripts for measurement study and characterization
